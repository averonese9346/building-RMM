#goal: to clean the data from the validation set from kaggle. making sure there are no duplicate
#entries, no null, etc.
...
def clean_text(s):
    """Normalize whitespace, quotes, dashes, newline markers, etc."""
    if s is None:
        return ""
...
print(f"Original dataset size: {len(valid_data)}")
...
with open("valid_cleaned.json", "w", encoding="utf-8") as f:
    json.dump(cleaned, f, indent=2, ensure_ascii=False)

print("\nSaved cleaned dataset → valid_cleaned.json")
